...
public class DogReducingBean {
    public static void main(String[] args) {
        ...
        System.out.println(dogs.stream().collect(Collectors.reducing(0.0, DogExtBean::getWeight, Double::sum)));

        System.out.print("Same, using summingDouble collector: ");
        System.out.println(dogs.stream().collect(Collectors.summingDouble(DogExtBean::getWeight)));
        System.out.print("Same, by map-reduce with DoubleStream::sum(): ");
        System.out.println(dogs.stream().mapToDouble(DogExtBean::getWeight).sum());
        System.out.println("---");

        System.out.print("Average age by reducing collector: ");
        int sum = dogs.stream().collect(Collectors.reducing(0, DogExtBean::getAge, Integer::sum));
        System.out.println(dogs.isEmpty() ? 0 : sum / (double) dogs.size());

        System.out.print("Same, using averagingInt collector: ");
        System.out.println(dogs.stream().collect(Collectors.averagingInt(DogExtBean::getAge)));
        System.out.print("Same, by map-reduce with IntStream::average(): ");
        dogs.stream().mapToInt(DogExtBean::getAge).average().ifPresent(System.out::println);
        System.out.println("---");

        System.out.println("All owners: " + dogs.stream().map(DogExtBean::getOwner).collect(Collectors.joining(", ")));
        System.out.print("All owners in brackets: ");
        System.out.println(dogs.stream().map(DogExtBean::getOwner).collect(Collectors.joining(", ", "[", "]")));
    }
}
